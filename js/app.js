(() => {
    ...
    if (menuBurger) menuBurger.addEventListener("click", (function(e) {
        menuBurger.classList.toggle("_active-icon");
        if (menuBody) menuBody.classList.toggle("_active-burger");
        if (wrapperBg) wrapperBg.classList.toggle("_active-bg");
        if (headerBurger) headerBurger.classList.toggle("_active-hidden");
    }));
    $(".button-volume").on("click", (function() {
        $(this).toggleClass("button_voice__mute ").hasClass("button_voice__mute");
        var muted = $("#video-item").prop("muted");
        $("#video-item").prop("muted", !muted);
    }));
    var header = $(".element-animation");
    ...
    $(window).scroll((function() {
        $(header).each((function() {
            if (true === isScrolledIntoView(this)) $(this).addClass("element-show");
        }));
    }));
    function fixedHed() {
        let lastScroll = 0;
        ...
        window.addEventListener("scroll", (() => {
            ...
            if (scrollPosition() < defaultOffset && !containHide()) header.classList.add("hide");
            lastScroll = scrollPosition();
        }));
    }
    fixedHed();
    const screen = document.querySelector(".sell-car");
    const screenItem = screen.querySelector(".sell-car__bg-l .sell-car__bg-l-wrapper");
	const screenItemTwo = screen.querySelector(".sell-car__bg-r .sell-car__bg-l-wrapper");
    ...
    function moveItemOnScroll() {
        ...
        const currentPointLeft = (windowWidth - screenItemSizes.width) / 250 * scrollProcent;
        screenItem.style.cssText = `\n\t\tbackground-position-x: ${currentPointLeft}px;\n\t\t;\n\t`;
		screenItemTwo.style.cssText = `\n\t\tbackground-position-x: ${currentPointLeft}px;\n\t\t;\n\t`;
    }
    window["FLS"] = true;
    isWebp();
    spollers();
